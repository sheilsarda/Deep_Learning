...
&p(w) = \frac{e^{\beta \Phi(w)}}{Z}  \implies Z \times p(w) = e^{\beta \Phi(w)} &(1)\\
&log(Z \times p(w)) = -\beta \Phi(w) \implies \Phi(w) = \frac{log(Z \times p(w))}{-\beta }  &(2)  \\
&\text{Substituting (1)  + taking partial w.r.t. $\beta$ on both sides: } \\
\frac{\partial log  Z}{\partial \beta} &= \sum_{w \in \mathcal{W}} Z \times p(w) \frac{log(Z \times p(w))}{\beta} \\
\frac{\partial log  Z}{\partial \beta} &= -Z \sum_{w \in \mathcal{W}} \frac{log(Z \times p(w))}{-\beta} \times p(w) \\
\end{align*}
